...
class MainMenu(Tkinter.Tk):
	...
	def createMenuItem(self): 		
		...
		self.menuTool.add_command(label="Bin/Excel self.converter", command=self.eventChangeMenuItem_Tool_BinExcelConverter)
	...
	def login(self):
		...
		#self.DBconnector = DBC.DatabaseConnector(self.tempLoginID.get(), self.tempLoginPWD.get()) #create an DB object
		self.DBconnector = DBC.DatabaseConnector("DataConverter.db") #create an DB object under dbName="DataConverter.db"
		STATUS = ""
		if(self.DBconnector.validateUserLogin(self.tempLoginID.get(), self.tempLoginPWD.get())):
			self.loginID = self.tempLoginID
			self.Msg = "Login successfully!"
			self.DBconnector.insertUserLoginLog("OK")
			tkMessageBox.showinfo("OK", self.Msg)
		else:
			self.loginID = ""
			self.Msg = "Try again!"
			self.DBconnector.insertUserLoginLog("ERROR")
	...
	#Bin/Excel self.converter
	def eventChangeMenuItem_Tool_BinExcelConverter(self):
		if(self.loginID == ""):
			tkMessageBox.showinfo("Error", "Please login system firstly!")
		else:
			self.menuAction = "Bin/Excel converter"
			self.converter = BEC.BinExcelConverter(self.sourcePath, self.templatePath)
			#OverRide object("self.converter") attribute
			if(self.configExcelSetting is not ""):
				self.converter.configExcelSetting = self.configExcelSetting
				self.converter.defaultProgramSetting()
			#start self.converter
			if(not self.converter.validationEncapsulation()):
				self.converter.writeLog()
				tkMessageBox.showinfo("Error", self.converter.message)
			else:			
				self.converter.convertingEncapsulation()#start converting to a new file.
				self.DBconnector.insertUserActionLog(self.menuAction, self.converter.backupFolder, self.converter.sourceFileName + "." + self.converter.sourceFileExtension, self.converter.backupFolder, self.converter.templateFileName + "." + self.converter.templateFileExtension, os.getcwd() + "\\resultData\\", self.converter.resultFolder.split('\\')[-1], self.converter.convertingItem, self.converter.configExcelSetting, "", "", "OK", "")
				print self.converter.resultFolder
				tkMessageBox.showinfo("OK", self.converter.message)
